...
const ScoreBoard = () => {
    ...
    useEffect(() => {
        const getRound = async () => {
            try {
                setIsLoading(true)
                const response = await jwtAxios.get(`${API_BASE_URL}/round`, { withCredentials: true })
    ...
    useEffect(() => {
        const getMatch = async () => {
            try {
                setIsLoading(true)
                const response = await jwtAxios.get(`${API_BASE_URL}/scoreboard?round_id=${currentRound}`, { withCredentials: true })
    ...
    return (
        <Box sx={{ flexGrow: 1, margin: '40px 10%' }}>
            ...
            {matches && matches.filter((match: Match) => match.round === currentRound).map((game: any) => <GameBox key={game.id} game={game} />)}
        </Box>
    )
}
