...
const UserInfo: React.FC<{ user: UserDetailData }> = ({ user }) => {
  ...
  const handleClick = () => {
    ...
    const isLeagueAdmin = userProfile.role?.name === UserRoleEnum.LeagueAdmin
  ...
  return (
    <Card
      ...
      onClick={handleClick}
    >
      <Avatar sx={{ width: 56, height: 56, textAlign: 'center', mb: '15px' }}>
        {user.name[0]}
      </Avatar>
      <Typography variant="subtitle1">name: {user.name}</Typography>
      <Typography variant="subtitle1">email: {user.email}</Typography>
      <Typography variant="subtitle1">
        Average Score: {user.playerMatchInfo?.averageScore}
      </Typography>
      <Typography variant="subtitle1" gutterBottom>
        Partipant Game Count: {user.playerMatchInfo?.gameCount}
      </Typography>
    </Card>
  )
}
